...
The rule API takes saved evaluator IDs and does not pin a function version in function_ids. Keep the evaluator unchanged during the rewind or use an immutable copy. If the filter or scorer scope does not validate, stop and report the error; do not broaden the backfill.
```
